...
export default function ModalUpdateContractor({ contractor_id }: Props) {
  ...
  const fetchData = async()=>{
    ...
    const resContract = await projectService.getContractorByID(contractor_id as string)
   
    if(resContract.statusCode === 200){
      form.setFieldsValue({...resContract.data,customer:resContract?.data?.customer?.info_id});
    }
  }
  ...
  const handleSubmit = async (values: ICreateContractor) => {
    ...
    const res = await postdata(() =>
      projectService.updateContractor(contractor_id as string,values)
    );
    if (res === 200 || res === 201) {
      form.resetFields();
      setIsModalVisible(false);
