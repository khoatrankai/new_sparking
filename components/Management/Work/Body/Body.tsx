...
export default function BodyWork() {
    ...
    const {type_tab} = useParams()
    const refBtnWork = useRef<HTMLButtonElement>(null)
    ...
    const [type,setType] = useState<'basic'|'kanban'|'gantt'>('kanban')

    const TabBarExtraContent= ()=>{
   
        return <div className='flex gap-1 items-center'>
            <Button className='flex flex-col items-center justify-center min-h-20' type='text' onClick={()=>{
              setType('kanban')
              router.push(`/management/all_work?id=${dataType[0].type_work_id}`)
              }}>
            <TbLayoutKanban className='min-text-xl'/>
            <span className='text-xs font-medium'>Kanban</span>
            </Button>
            <Button className='flex flex-col items-center justify-center min-h-20' type='text' onClick={()=>{setType('basic')}}>
            <CiBoxList className='text-xl'/>
            <span className='text-xs font-medium'>Hiển thị</span>
            </Button>
            <Button className='flex flex-col items-center justify-center min-h-20' type='text' onClick={()=>{setType('gantt')}}>
            <FaChartGantt className='text-xl'/>
            <span className='text-xs font-medium'>Gantt chart</span>
            </Button>
            <Button className='flex flex-col items-center justify-center min-h-20' type='text' onClick={()=>{refBtnWork.current?.click()}}>
            <IoIosAdd className='text-xl'/>
            <span className='text-xs font-medium'>Thêm việc</span>
            </Button>
            <Button className='flex flex-col items-center justify-center min-h-20' type='text'>
    ...
    const tabs: TabsProps["items"] = [
      ...
      useEffect(()=>{
      if(type_tab){
        setType(type_tab as ('basic'|'kanban'|'gantt'))
      }
      },[type_tab])
  return (
    <div>
