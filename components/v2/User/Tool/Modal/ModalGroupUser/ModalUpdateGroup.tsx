...
export default function ModalUpdateGroup({refBtnUpdate,name_group,ID}:Props) {
  ...
  const handleSubmit = async (values: IGetGroupUser) => {
    const res = await postdata(() => userService.updateGroupUser(ID as string,values));
    if (res === 200 || res === 201) {
      setIsModalVisible(false);
  ...
  useEffect(()=>{
    if(name_group){
      form.setFieldValue('name_group',name_group)
    }
  },[name_group])
