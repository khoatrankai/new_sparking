...
export default function ChartColumnProject() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (typeTime || timeEnd || timeFirst) {
        const filter = {
          time_start: timeFirst,
          time_end: timeEnd
        };
        const res = await projectService.getFilterProject(filter);
        if (res.statusCode === 200) {
          const dataSource: IGetProject[] = res.data;
          if (typeTime === "day") {
            const dataDay = dataSource.reduce((value: any, item) => {
              const dateTime = new Date(item.start_date ?? "");
          ...
          if (typeTime === "week") {
            const dataWeek = dataSource.reduce((value: any, item) => {
              const dateTime = moment(item.start_date ?? "");
  ...
  return (
    <div className="sm:px-4 py-4 flex flex-col gap-4 max-w-full">
      ...
      <div className="w-[731px] h-[256px] overflow-hidden">
        <DemoColumn />
      </div>
    </div>
  );
