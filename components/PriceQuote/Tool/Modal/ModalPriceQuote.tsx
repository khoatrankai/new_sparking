...
export default function ModalAddPriceQuote({refBtn,idOpportunity}:Props) {
  ...
  const handleSubmit = async (values: ICreatePriceQuote) => {
    const res = await postdata(() =>
      priceQuoteService.createPriceQuote({
        ...values,
        opportunity:idOpportunity ?? undefined,
  ...
  const handlePushPart = async () => {
    setListPart((preValue) => {
      return [
        ...preValue,
        { title: "", products: [], type_package: undefined },
      ];
    });
  };
...
<Form.Item
        ...
        <div className="flex flex-col items-end gap-2 p-2 w-full bg-slate-100 rounded-sm">
          ...
          <div>
            <div className="flex sm:items-center justify-end items-end">
              <div className="sm:w-80 flex items-center justify-end gap-2 flex-wrap">
                Chiết khấu{" "}
