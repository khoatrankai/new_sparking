...
import ModalListPriceQuote from "./Modal/ModalListPriceQuote";

export default function ListOpportunityByPriceQuote() {
  ...
  const refBtnPriceQuote = useRef<HTMLButtonElement>(null)
  const refBtnListPriceQuote = useRef<HTMLButtonElement>(null)
  const { datas: dataSource } = useSelector(
    (state: RootState) => state.get_opportunities_by_price_quote
  );
          ...
          <div className="flex gap-2">
            ...
            {isAuthorized && (
              ...
              <Button type="text" ghost className="text-xs text-blue-600" onClick={()=>{
                ...
                if(refBtnListPriceQuote.current)
                  refBtnListPriceQuote?.current.click()
              }}>
              Danh sách báo giá
            </Button>
                <ModalUpdateOpportunity ID={value} />
                
                <Button type="text" ghost className="text-xs text-green-600" onClick={()=>{
                setIdOpportunity(value)
  ...
  return (
    <div className="">
      ...
      <ModalListPriceQuote idOpportunity={idOpportunity} refBtn={refBtnListPriceQuote as Ref<HTMLButtonElement>}/>
    </div>
  );
}
