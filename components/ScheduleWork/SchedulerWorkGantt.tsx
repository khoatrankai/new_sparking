...
import { fetchYearWorks } from "@/redux/store/slices/activitySlices/work.slide";
...
export default function SchedulerWorkGantt() {
  const dispatch = useDispatch<AppDispatch>();

  useEffect(() => {
    dispatch(fetchYearWorks(new Date().getFullYear()));
  }, [dispatch]);
  const { datas: dataWork } = useSelector(
    (state: RootState) => state.get_works
  );
  const getListData = (value: Dayjs) => {
    return dataWork.filter((dt) => {
  ...
  const dateCellRender = (value: Dayjs) => {
    ...
    return (
      <ul className="flex flex-col gap-2 px-2">
        {listData.map((item) => (
          <>
            <li key={item.work_id} className="p-2 rounded-lg  bg-red-100/30">
              <Popover
                title={"Chi tiết"}
                content={
                  <>
