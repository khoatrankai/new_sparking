...
const contractService = {
  ...
  getContract: async (id: string) => {
    ...
    getSalesByProvince: async (filters?:{ids:string,timestamp_start:string,timestamp_end:string}) => {
    try {
      const queryParams = new URLSearchParams();
      ...
      const response = await api.get(`/contract/get-sales-by-province?${queryParams.toString()}`);
      return response.data;
    } catch (error) {
      handleError(error);
      throw error;
  ...
  getDashboardRevenue: async (year:number) => {
    try {
      const response = await api.get(`/contract/get-dashboard-revenue?year=${year}`);
      return response.data;
    } catch (error) {
      handleError(error);
      throw error;
